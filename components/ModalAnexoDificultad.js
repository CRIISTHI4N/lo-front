...
export default function ModalAnexoDificultad() {
    ...
    const tomarFoto = async (e, dispositivo) => {
        ...
        if (dispositivo === "movil" || dispositivo === "desktop_input") {
            ...
            for (let i = 0; i < imagenes.length; i++) {
                ...
                nuevasCapturas.push({ id: generarId(), fotoAnexoUrl: file });
            }
            ...
            canvas.toBlob((blob) => {
                ...
                const newImage = { id: Date.now(), fotoAnexoUrl: blob };
    ...
    const eliminarImagen = (id) => {
        ...
        if (editarContext) {
            const anexo = anexoDificultad.find(a => a.id === idAnexoContext);

            if (anexo.fotosActividad.length) {
                const imagenesFiltradas = anexo.fotosActividad.filter(q => q.id !== id);
                anexo.fotosActividad = imagenesFiltradas
            }
        } else {
    ...
    const enviarDatos = () => {
        ...
        if (editarContext) {
            ...
            anexoDificultad.map(a => {
                if (a.id === idAnexoContext) {
                    a.fotosAnexo = capturas;
                }
            });
            ...
            setCapturasContextAnexo(capturas);
        }
    ...
    useEffect(() => {
        ...
        if (capturasContextAnexo) {
            setCapturas(capturasContextAnexo);
        }
    ...
    return (
        <>
            <div className="py-10 px-5 border-b border-gray-300">
                ...
                <span className="block text-center text-2xl">Fotografías Anexo Dificultad</span>
            </div>
            ...
            <div className="py-10 px-12" >
                    ...
                    {capturas.map(c =>
                        ...
                        >
                            <img
                                ...
                                src={URL.createObjectURL(c.fotoAnexoUrl)}
                            />
                ...
                <div className="grid gap-5 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                    ...
                    {editarContext && capturasContextAnexo && capturasContextAnexo.length &&

                        capturasContextAnexo.map(c =>
                            <div
                                key={c.id}
                                className="w-full relative"
                            >
